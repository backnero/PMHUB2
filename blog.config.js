...
const BLOG = {
  ...
  NOTION_PAGE_ID: process.env.NOTION_PAGE_ID || '3b1cfab62aa74522ba29828f2c548f56',
  PSEUDO_STATIC: false, // 伪静态路径，开启后所有文章URL都以 .html 结尾。
  NEXT_REVALIDATE_SECOND: 15, // 更新内容缓存间隔 单位(秒)；即每个页面有5秒的纯静态期、此期间无论多少次访问都不会抓取notion数据；调大该值有助于节省Vercel资源、同时提升访问速率，但也会使文章更新有延迟。
  ...
  AUTHOR: 'PMHub', // 作者
  BIO: 'PMer Knowledge Sharing', // 作者简介
  LINK: 'https://pmhub.app/', // 网站地址
  KEYWORDS: 'PMHUB,Product,Product Management', // 网站关键词 英文逗号隔开
  ...
  ANALYTICS_GOOGLE_ID: process.env.NEXT_PUBLIC_ANALYTICS_GOOGLE_ID || 'G-GRG0BG3LDT', // 谷歌Analytics的id e.g: G-XXXXXXXXXX
  ...
  // 谷歌广告
  ADSENSE_GOOGLE_ID: process.env.NEXT_PUBLIC_ADSENSE_GOOGLE_ID || 'ca-pub-8522617122947382', // 谷歌广告ID e.g ca-pub-xxxxxxxxxxxxxxxx
  ...
  TITLE: process.env.NEXT_PUBLIC_TITLE || 'PMHub', // 站点标题 ，被notion中的页面标题覆盖
  HOME_BANNER_IMAGE: './bg_image.jpg', // 首页背景大图, 会被notion中的封面图覆盖，若无封面图则会使用代码中的 /public/bg_image.jpg 文件
  DESCRIPTION: process.env.NEXT_PUBLIC_DESCRIPTION || 'PMer Knowledge Sharing', // 站点描述，被notion中的页面描述覆盖
