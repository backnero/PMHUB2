  ...
  for (let i = 0; i < rawProperties.length; i++) {
    ...
    if (schema[key]?.type && !excludeProperties.includes(schema[key].type)) {
      ...
      switch (schema[key]?.type) {
        ...
        // case 'image': {
        //   const imgRaw = getImageUrl(val)
        //   contentImgList.push(imgRaw)
        // }
        default:
          break
      }
...
// console.log(contentImgList)
  ...
  properties.page_cover = getImageUrl(block[id].value?.format?.page_cover, block[id].value)?? 'https://www.pmhub.app/_next/image?url=https%3A%2F%2Fwww.notion.so%2Fimage%2Fhttps%253A%252F%252Fwww.notion.so%252Fimages%252Fpage-cover%252Fnasa_earth_grid.jpg%3Ftable%3Dblock%26id%3D71809768-9305-4328-a7b9-8d78838cf71a%26cache%3Dv2&w=2048&q=75' ?? siteInfo?.pageCover
